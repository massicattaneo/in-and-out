...
const startOfTheWeek = date => {
  ...
  newDate.setTime(newDate.getTime() - newDate.getDay() * 24 * 60 * 60 * 1000)
  return newDate
}

const weekDifference = (from, to) => {
...
function getDayWorkers({ calendars }, date) {
  ...
  const day = calendar[new Date(date).getDay()]
...
function sortByDate(field, order = -1) {
  return function (a, b) {
    return (
      -order * new Date(b[field].split("/").reverse().join("-")).getTime() +
      order * new Date(a[field].split("/").reverse().join("-")).getTime()
    )
  }
}
...
function getPromotionDiscounts(promotion) {
  return (promotion.discounts || "").split(",").map(d => {
    ...
    const items = d
      ...
      .map(i => {
        return {
          count: Number(i.split("x")[0]),
          id: i.split("x")[1],
          type: i.split("x")[1].substr(0, 3),
        }
      })
      ...
      workerHours
        ...
        .forEach(function (wh) {
          for (let h = wh[2]; h <= wh[3] - duration; h += 0.25) {
            ...
            if (
              filter.length === busy.length &&
              hStart.getTime() > new Date(serverTimestamp).getTime()
            ) {
              hours.push(h)
            }
          }
...
module.exports = {
        ...
        Object.assign(
          {
            available:
              day.filter(
                arr =>
                  arr.length && Number(t[workers[arr[1]].column]) > 0 && wks.indexOf(arr[1]) !== -1,
              ).length > 0 && t[centerId] !== "0",
          },
          t,
        ),
      )
  ...
  getLocation: function (db, date, googleId) {
    ...
    const filter = cal[bookDate.getDay()]
  ...
  getCartTotal: function cartTotal(store, cart) {
    ...
    if (activePromotions(store.promotions).filter(p => p.discounts.indexOf("*") !== -1).length) {
      const allItemsDiscount = activePromotions(store.promotions).filter(
        p => p.discounts.indexOf("*") !== -1,
      )
      const all = allItemsDiscount.map(item => item.discounts)
      const typeDiscounteds = all.map(ii =>
        ii.split("&").map(d => [d.match(/\d*x(.*)-\*/)[1], Number(d.match(/\d*x\w*-\*=(.*)/)[1])]),
      )
      ...
      typeDiscounted.forEach(function ([type, disc]) {
        cart.forEach(function (id) {
          ...
          const isDisc =
            !isBiologique(it) && !Number(it.credito) > 0 && id.match(/(.*)-\d*/)[1] === type
          discounted += isDisc ? tot * disc : 0
          real += isDisc ? tot : 0
        })
      })
    }
    const items = activePromotions(store.promotions).reduce(
      function ({ discounted, notDiscounted }, p) {
  ...
  toICSDate: function toICSDate(date) {
    ...
    return new Date(tr.getTime() + getSpainOffset(date) * 60 * 60 * 1000)
      .toISOString()
      .replace(/-/g, "")
      .replace(/:/g, "")
      .substr(0, 15)
