...
const getUser = (what, centers) => {
  const [user = null] = centers
...
const getLine = (str) => {
  ...
  while (
    !part.match(/\d\d\/\d\d\/\d\d\d\d\d\d\/\d\d\/\d\d\d\d$/) &&
    length <= str.length
  ) {
...
const getBBVAEntries = (start, centers) => {
  let list = start;
  const regExp =
    /(-?[\d\.,]*) EUR-?([\d\.,]*) EUR(.*)(\d\d\/\d\d\/\d\d\d\d)(\d\d\/\d\d\/\d\d\d\d)$/;
  const cashEntries = [];
  ...
  while (getLine(list)) {
    ...
    if (description.match(/LIQUID.LIQUIDA REMESAS COMERCICOMERC (.*)/)) {
      ...
      newDate.setTime(
        newDate.getTime() - (newDate.getDay() === 1 ? 3 : 1) * ONE_DAY
      );
      cashEntries.push({
        amount,
        description,
        ...
        type: "pos-deposit",
        user: getUser(description, centers),
      });
