...
const app = express()
const googleModule = require("./google-api")
const mailer = require("./mailer/mailer")()
const isDeveloping = process.env.NODE_ENV === "development"
const mongoModule = require("./mongo")
const mongo = mongoModule(isDeveloping, utils)
...
async function sendCommunication(
  ...
  if (type === "email") {
    const { _id } = await mongo.rest.insert("communications", {
      type,
      action,
      clientId,
      date: Date.now(),
    ...
    mailer
      .send(createTemplate(action, body))
      .then(() => {
        onSuccess("ok")
      })
      .catch(async () => {
...
;(async function () {
  ...
  const google = googleModule(utils, posts, mongo)
  ...
  app.get("/api/posts/*", function (req, res) {
    ...
    const images = post
      ? posts.filter(p => p.post_parent === post.ID).filter(p => p.post_type === "attachment")
      : []
  ...
  app.get("/api/public-db", async function (req, res) {
    const googleDb = await google.publicDb()
    res.send({
      ...googleDb,
  ...
  app.post("/api/treatments/favourite", requiresLogin, function (req, res) {
    mongo
      .favouriteTreatment({
  ...
  app.post("/api/reviews", requiresLogin, function (req, res) {
    mongo
      .insertReview({
        rate: req.body.rate,
        lang: req.body.lang,
  ...
  app.post("/google/get-hours", async function (req, res) {
    ...
    const workers =
      center !== undefined
        ? shared.getWorkers(googleDb, date, center)
        : shared.getDayWorkers(googleDb, date)
  ...
  app.post("/api/stripe/secret", async function (req, res) {
    ...
    const googleDb = await google.publicDb()
    const cartAmount = shared.getCartTotal(googleDb, cart).total
    const productsAmount = shared.getCartTotal(
      googleDb,
      cart.filter(id => id.substr(0, 3) === "PRD"),
    ).total
    const freeChargeLimit = googleDb.settings.freeChargeLimit
    const sendingCharge = googleDb.settings.sendingCharge
    const amount = Math.floor(
      (cartAmount +
        (productsAmount < freeChargeLimit && productsAmount !== 0 ? sendingCharge : 0)) *
        100,
    )
  ...
  app.post("/api/stripe/payment-done", async function (req, res) {
    const googleDb = await google.publicDb()
    const { email, privacy, orderId, paymentIntent } = req.body
    const cart = req.body.cart.map(id => Object.assign({ id, used: false }))
    const cartAmount = shared.getCartTotal(googleDb, req.body.cart).total
    const productsAmount = shared.getCartTotal(
      googleDb,
      req.body.cart.filter(id => id.substr(0, 3) === "PRD"),
    ).total
    const freeChargeLimit = googleDb.settings.freeChargeLimit
    const sendingCharge = googleDb.settings.sendingCharge
    const amount = Math.floor(
    ...
    try {
      ...
      if (req.session.userId) {
        try {
          ;[userFromSessionId] = await mongo.rest.get(
            "users",
  ...
  app.post("/api/communications/preview/:type/:action", requiresAdmin, async function (req, res) {
  ...
  app.post(
    "/api/communications/send/:type/:action/:clientId",
    requiresAdmin,
  ...
  app.get("/api/pdf/bill/:clientId/:timestamp", async function (req, res) {
    ...
    const cashEntries = await mongo.rest.get(
      "cash",
      `clientId=${clientId}&date>${from.getTime()}&date<${to.getTime()}`,
    )
    const user = ((await mongo.rest.get("users", `_id=${clientId}`)) || [])[0]
    if (!user || cashEntries.length === 0 || !cashEntries[0].billInfo || !cashEntries[0].billNumber)
      return res.send("<html><body>ESTA FACTURA NO EXISTE</body></html>")
    const formatted = formatItemForPdfBill(
      cashEntries,
      cashEntries[0].billNumber,
      formatDateShort(from),
      cashEntries[0].billInfo,
    )
    createPdfBills(res, [formatted])
  })

  app.get("/api/last-bill-numbers", requiresAdmin, async (req, res) => {
    const response = await mongo.getLastBillNumbers()
    res.json(response)
  })

  app.post("/api/download-bills", requiresAdmin, async function (req, res) {
    const { billNumbers } = req.body
    const array = await Promise.all(
      billNumbers.map(async billNumber => {
        const cashEntries = await db
          .collection("cash")
          .aggregate([{ $match: { billNumber: Number(billNumber) } }])
          .toArray()
        return formatItemForPdfBill(
          cashEntries,
          billNumber,
          formatDateShort(new Date(cashEntries[0].date)),
          cashEntries[0].billInfo,
        )
      }),
    )
  ...
  app.post("/api/generate-bills", requiresAdmin, async function (req, res) {
    const { ids } = req.body
    const cashItems = (
      await Promise.all(
        ids.map(async id => {
    ...
    await cashItems
      .reduce(async (prev, cashItem) => {
        ...
        if (cashItem.clientId) {
          ...
          cashEntries = await db
            .collection("cash")
            .aggregate([
              {
                $match: {
                  clientId: cashItem.clientId,
                  date: { $gte: from.getTime(), $lte: to.getTime() },
                },
              },
            ])
            .toArray()
        ...
        await Promise.all(
          cashEntries.map(cash => mongo.rest.update("cash", cash._id, { billNumber, billInfo })),
        )
      }, Promise.resolve())
      .then(() => {
        res.json({})
  ...
  app.get("/api/bills/cash/:clientId/:timestamp", requiresAdmin, async function (req, res) {
    const { clientId, timestamp } = req.params
    ...
    const billInfo = Object.assign(
      { nif, cap, address, city },
  ...
  app.get("/api/pdf/*", async function (req, res) {
    const googleDb = await google.publicDb()
    ...
    if (orderId === "exemplo") {
      createPdfOrder(res, googleDb, orderId, [])
    } else {
      const { cart } = await mongo.getOrderInfo(orderId)
      createPdfOrder(res, googleDb, orderId, cart)
    }
  })

  app.get("/api/orders/products/unsent", requiresAdmin, async (req, res) => {
    return res.json(await mongo.unsentProductOrders())
  })

  app.post("/google/calendar/insert", requiresLogin, async function (req, res) {
    const googleDb = await google.publicDb()
    ...
    google
      .calendarInsert({
        ...
        to: new Date(
          dateMin.getTime() +
            shared.getTreatmentsDuration(googleDb, all, treatments, workerIndex) * 60 * 1000,
        ).toISOString(),
  ...
  app.post("/google/bank-holidays", requiresAdmin, async function (req, res) {
    res.json(google.bankHolidays(req.body.year))
  })

  app.post("/google/calendar/delete", requiresLogin, async function (req, res) {
    google
  ...
  app.post("/google/calendar/get-all", requiresAdmin, async function (req, res) {
  ...
  app.post("/google/calendar/get", requiresAdmin, async function (req, res) {
  ...
  app.post("/google/calendar/add", requiresAdmin, async function (req, res) {
    const {
      duration,
      calendarId,
      date,
  ...
  app.get("/api/qr-code/*", async function (req, res) {
  ...
  app.get("/api/adminDb", requiresAdmin, async function (req, res) {
    res.send({
  ...
  app.get("/api/actual-cash", requiresAdmin, async function (req, res) {
    res.send({
      salitre: Math.min(4000, await mongo.getActualCash("salitre")),
      buenaventura: Math.min(4000, await mongo.getActualCash("buenaventura")),
      portanueva: Math.min(4000, await mongo.getActualCash("portanueva")),
  ...
  app.post("/google/user/bookings", requiresAdmin, async function (req, res) {
    const { email } = req.body
    // const data = await google.getBookings(email)
    res.json([])
  })

  app.get("/api/rest/*", requiresAdmin, async function (req, res) {
  ...
  app.post("/api/rest/*", requiresAdmin, async function (req, res) {
    const paths = req.url.split("/")
    const table = paths[paths.length - 1]
    const ip = req.headers["x-forwarded-for"] || req.connection.remoteAddress
    mongo.rest
      .insert(table, Object.assign(req.body, { ip }))
      .then(function (cash) {
        res.send(cash)
      })
      .catch(function (err) {
  ...
  app.put("/api/rest/*", requiresAdmin, async function (req, res) {
    const paths = req.url.split("/")
    const table = paths[paths.length - 2]
    const id = paths[paths.length - 1]
    mongo.rest
      .update(table, id, req.body)
      .then(function (cash) {
        res.send(cash)
      })
      .catch(function (err) {
  ...
  app.delete("/api/rest/*", requiresAdmin, async function (req, res) {
    const paths = req.url.split("/")
    const table = paths[paths.length - 2]
    const id = paths[paths.length - 1]
    mongo.rest
  ...
  app.post("/api/newsletter", requiresAdmin, async function (req, res) {
    if (req.body.test) {
  ...
  app.post("/api/email/:type", requiresAdmin, async function (req, res) {
  ...
  app.get("/api/upload/*", requiresAdmin, async function (req, res) {
  ...
  app.post("/api/upload-bank", requiresAdmin, async function (req, res) {
    const accounts = {
      main: "0201602867",
    ...
    for (let i = 0; i < docs.length; i++) {
      ...
      await new Promise(resolve =>
        db.collection("bank").update(doc, doc, { upsert: true }, resolve),
      )
    }
    res.send({})
  })

  app.post("/api/upload", requiresAdmin, async function (req, res) {
    const name = req.files.fileUpload.name
    const ext = path.extname(name)
    google
      .upload(name, req.files.fileUpload.data, ext)
      .then(async function (googleRef) {
  ...
  app.delete("/api/upload/:id", requiresAdmin, async function (req, res) {
  ...
  app.post("/api/bbva/updload", requiresAdmin, async function (req, res) {
  ...
  app.get("/api/cash/summary", requiresSuperAdmin, async function (req, res) {
    const data = await mongo.cashSummary()
    res.json(data)
  })

  app.get("/api/cash/comparing-summary", requiresSuperAdmin, async function (req, res) {
  ...
  app.post("/api/client/error", bruteforce.prevent, async function (req, res) {
    if (req.headers.origin === "https://www.inandoutbelleza.es") {
      ...
      mailer.send({
        ...
        html: Object.keys(req.body)
          ...
          .map(key => `<p>${key.toUpperCase()}: ${req.body[key]}</p>`)
          .join(""),
      })
    }
    res.json({})
      ...
      if (isSpider) {
        try {
          if (req.path.endsWith("/")) {
            res.write(
              fs.readFileSync(path.join(__dirname, `static/seo${req.path}index.html`), "utf8"),
            )
          } else {
