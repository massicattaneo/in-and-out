...
export default async function({ system, thread, locale }) {
    ...
    const model = { cash: [], bonus: [] }.reactive();
    ...
    ({ cash: () => model.cash, adminLevel: () => system.store.adminLevel })
        .reactive()
        .connect(({ cash, adminLevel }) => {
            if (adminLevel === 2) {
    ...
    ({ cash: () => model.cash, bonus: () => model.bonus, adminLevel: () => system.store.adminLevel })
        .reactive()
        .connect(({ cash, bonus, adminLevel }) => {
