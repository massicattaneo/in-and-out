...
export default async function ({ system, parent, context, thread }) {
    ...
    const item1 = system.store.news.map(i => {
        return {
            ...
            info: i.descripcion.substr(0, 150) + `... <br/><a onclick="window.navigate(this)" href="/es/novedades/${i.href}">LEER MAS</a>`,
        }
    })[0];
    const item2 = system.store.promotions.map(i => {
        return {
            title: i.titulo, src: `/google/drive/promociones/${system.deviceInfo().deviceType}.${i.foto}`,
            info: i.descripcion.substr(0, 150) + `... <br/><a onclick="window.navigate(this)" href="/es/promociones/${i.href}">LEER MAS</a>`,
        }
    })[0];
    const item3 = system.store.press.map(i => {
        return {
            title: i.titulo, src: `/google/drive/press/${system.deviceInfo().deviceType}.${i.foto}`,
            info: i.descripcion.substr(0, 150) + `... <br/><a onclick="window.navigate(this)" href="/es/en-los-medios/${i.href}">LEER MAS</a>`,
        }
    })[0];
